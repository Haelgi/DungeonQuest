...
const treasure_cards = [
    new Card(1, 'Кольцо Жизни', 250, ()=>{ringOfLife()}),
    new Card(2, 'Малый Кристал Магии', 200, ()=>{smallMagicCrystal()}),
    new Card(3, 'Ожерелье с Сапфирами', 250, ()=>{sapphireNecklace()}),
    new Card(4, 'Магическое Кольцо', 290, ()=>{magicRing()}),
    new Card(5, 'Большой Кристалл Магии', 320, ()=>{largeMagicCrystal()}),
    new Card(6, 'Волшебный Ключ', 350, ()=>{magicKey()}),
    new Card(7, 'Огненный Амулет', 400, ()=>{fireAmulet()}),
    new Card(8, 'Змеиное Кольцо', 400, ()=>{snakeRing()}),
    new Card(9, 'Кинжал Скорости', 450, ()=>{speedDagger()}),
    new Card(10, 'Зелье Прозорливости', 450, ()=>{foresightPotion()}),
    new Card(11, 'Зелье Скорости', 450, ()=>{speedPotion()}),
    new Card(12, 'Сумеречная Накидка', 500, ()=>{twilightCloak()}),
    new Card(13, 'Амулет Теней', 550, ()=>{shadowAmulet()}),
    new Card(14, 'Яйцо Дракона', 600, ()=>{dragonEgg()}),
    new Card(15, 'Посох Жизни', 650, ()=>{staffOfLife()}),
    new Card(16, 'Посох Смерти', 650, ()=>{staffOfDeath()}),
    new Card(17, 'Пояс Феникса', 700, ()=>{phoenixBelt()}),
    new Card(18, 'Тиара Магнетизма', 700, ()=>{magnetismTiara()}),
    new Card(19, 'Арфа Спокойствия', 700, ()=>{tranquilityHarp()}),
    new Card(20, 'Пояс Жизни', 900, ()=>{lifeBelt()}),
    new Card(21, 'Меч Света', 900, ()=>{lightSword()}),
    new Card(22, 'Корона', 1000, ()=>{crown()}),
    new Card(23, 'Сумка с Самоцветами', 1000, ()=>{bagOfGems()}),
    new Card(24, 'Лампа с Джином', 1100, ()=>{genieLamp()}),
    new Card(25, 'Посох Дракона', 1510, ()=>{dragonStaff()}),
    new Card(26, 'Заколдованная Книга', false, ()=>{enchantedBook()}),
    new Card(27, 'Пожиратель Сокровищ', false, ()=>{treasureEater()}),
    new Card(28, 'Украшенные Ботинки', 90, ()=>{decoratedBoots()}),
    new Card(29, 'Кольцо с Кристалом', 110, ()=>{crystalRing()}),
    new Card(30, 'Жемчужное Кольцо', 130, ()=>{pearlRing()}),
    new Card(31, 'Зелье Внимательности', 150, ()=>{attentivenessPotion()}),
    new Card(32, 'Серебрянное Кольцо', 160, ()=>{silverRing()}),
    new Card(33, 'Кольцо Колдуна', 170, ()=>{sorcererRing()}),
    new Card(34, 'Золотое Кольцо', 190, ()=>{goldRing()}),
    new Card(35, 'Деревянное Кольцо', 1, ()=>{woodenRing()}),
    new Card(36, 'Свод Законов', 15, ()=>{lawBook()})
];
