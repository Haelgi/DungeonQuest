...
function lifeBelt() {
    ...
    ew.removeAllEW();
    /* "трофей"
    После каждого боя с монстром, который закончился смертью монстра, 
...
function lightSword() {
    ...
    ew.removeAllEW();
    /* "трофей"
    Пока эта карта у Вас, в бою с демоном, или скелетом, 
...
function sorcererRing() {
    ...
    ew.removeAllEW();
    /* "трофей"
    Бросив кубик, Вы можете сбросить эту карту (кольцо уничтожается) и добавить 1 к выпавшему на кубике результату. 
