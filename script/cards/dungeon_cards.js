...
const dungeon_cards = [
    ...
    /*25*/new Card(6, 'Пусто', ()=>{ew.removeAllEW() /*Комната пуста; ничего не происходит.*/}, 'Далі'),
    /*26*/new Card(6, 'Пусто', ()=>{ew.removeAllEW() /*Комната пуста; ничего не происходит.*/}, 'Далі'),
    
    /*27*/new Card(7, 'Захоронение', ()=>{burial()}, 'Далі'),
    /*28*/new Card(7, 'Захоронение', ()=>{burial()}, 'Далі'),
    
    /*29*/new Card(8, 'Обрушение Стен', ()=>{return/*Комната рушится. Выполните проверку Ловкости и, в случае неудачи, получите 2 ранения от летящих обломков. Вы обязаны выполнить еще одно перемещение в соседнюю область. Если она не исследована, разместите тайл комнаты в обычном порядке. Вы можете игнорировать двери и решетки. Если покинуть комнату не удалось, Вы погибаете. Уберите с поля тайл комнаты, которую Вы должны были покинуть, оставив на его месте неисследованную область.*/}, 'Далі'),
    /*30*/new Card(8, 'Обрушение Стен', ()=>{return/*Комната рушится. Выполните проверку Ловкости и, в случае неудачи, получите 2 ранения от летящих обломков. Вы обязаны выполнить еще одно перемещение в соседнюю область. Если она не исследована, разместите тайл комнаты в обычном порядке. Вы можете игнорировать двери и решетки. Если покинуть комнату не удалось, Вы погибаете. Уберите с поля тайл комнаты, которую Вы должны были покинуть, оставив на его месте неисследованную область.*/}, 'Далі'),
    
    /*31*/new Card(9, 'Подземный Некрополь', ()=>{return/*Вы нашли подземный некрополь. Вы можете вытянуть 3 Карты Склепа.*/}, 'Далі'),
    /*32*/new Card(9, 'Подземный Некрополь', ()=>{return/*Вы нашли подземный некрополь. Вы можете вытянуть 3 Карты Склепа.*/}, 'Далі'),
    
    /*33*/new Card(10, 'Гоблин-Исследователь', ()=>{return/*Вы встретили гоблина-исследователя. В обмен на 2 Ваших Трофея он готов показать секретный проход. Если хотите, можете сбросить 2 своих Трофея и немедленно переместиться в любую соседнюю область. Если она не исследована, разместите тайл комнаты в обычном порядке.*/}, 'Далі'),
    /*34*/new Card(10, 'Гоблин-Исследователь', ()=>{return/*Вы встретили гоблина-исследователя. В обмен на 2 Ваших Трофея он готов показать секретный проход. Если хотите, можете сбросить 2 своих Трофея и немедленно переместиться в любую соседнюю область. Если она не исследована, разместите тайл комнаты в обычном порядке.*/}, 'Далі'),
