...
const dwarf = new Hero('dwarf', 'Гном-Воїн Тарвін', 0, 9, 4, 5, 4, 20, function special() {
    //  если вытащили карту "секретный проход", то вы получаете 1 жетон решимости
    //  если вытащили карту "гоблин-исследователь", то можно переместиться в любую соседнюю область, сбросив 1 трофей, вместо 2.
    console.log(`special`)
}, hero_card_abilitie['dwarf']);

const enchantress = new Hero('enchantress','Чародійка Арвен', 0, 5, 7, 5, 7, 13, function special() {
    //  если разиграли карту "малое зелье лечения", то вы исцеляетесь не 2, а 3 ранения
    //  если побеждаете в бою с магом, то получаете 1 жетон решимости
    console.log(`special`)
}, hero_card_abilitie['enchantress']);

const hunter = new Hero('hunter','Мисливець Фарн', 0, 6, 6, 4, 5, 16, function special() {
    //  если тянете карту "ловушки", то получаете 1 жетон решимости
    //  если входит в катакомбы, то сразу тянешь карту катакомб
    //  если вытащили карту катакомбы "капкан", то она сбрасывается и тянется следующая
    console.log(`special`)
}, hero_card_abilitie['hunter']);

const knight = new Hero('knight','Рицар Хьюгарт', 0, 7, 5, 9, 4, 17, function special() {
    //  если тянете карту "монстра", то получаете 1 жетон решимости
    //  если побеждаете "монстра" без ранений, то получаете 1 жетон решимости
    //  после получения ранения от "ярости дракона", исцеляется 1 ранение
    console.log(`special`)
}, hero_card_abilitie['knight']);

const mage = new Hero('mage','Маг Геріон', 0, 3, 5, 4, 9, 14, function special() {
    //  перед началом боя бросьте 1д4, если выпало 4-6 противник получает 1 ранение молнией
