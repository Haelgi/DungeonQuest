...
const mage = [
    ...
    new Card( 3, 'Лечение', ()=>{
        ...
        ew.drawEW('Ви зцілили 4 здоровʼя')
        setTimeout(ew.removeAllEW, 1200);
        /*  Сбросьте эту карту в время своего хода. У Вас исцеляется 4 ранения.*/
        }),
...
const robber = [
    ...
    new Card( 2, 'Побег', ()=>{
            ew.drawEW('Не можна викорасти карту зараз(')
            setTimeout(ew.removeAllEW, 1200);  
        /*  Сбросьте эту карту, чтобы убежать во время боя с монстром не выполняя проверки Ловкости. 
            При этом Вы не получаете штраф за побег.*/
        }),
    
    new Card( 3, 'Легкая Поступь', ()=>{
            ew.drawEW('Не можна викорасти карту зараз(')
            setTimeout(ew.removeAllEW, 1200);  
        /*  Сбросьте эту карту после того как Вы вытянули Карту Ловушки. 
            Сбросьте Карту Ловушки не разыгрывая ее эффекта.*/
    ...
    new Card( 4, 'Шестое Чувство', ()=>{
            ew.drawEW('Не можна викорасти карту зараз(')
            setTimeout(ew.removeAllEW, 1200);  
        /*  Способность может применяться к любой колоде карт, кроме колоды Карт Дракона. 
